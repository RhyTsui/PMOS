...
const configuredTargets = process.env.FRONTEND_STYLE_AUDIT_TARGETS
  ? process.env.FRONTEND_STYLE_AUDIT_TARGETS.split(',')
      .map((item) => item.trim())
      .filter(Boolean)
  : null;
const targets = configuredTargets && configuredTargets.length > 0 ? configuredTargets : ['src/frontend'];
