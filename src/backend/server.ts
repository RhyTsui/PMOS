...
async function loadTaskWithWorkflowRun(taskId: string, subprojectId: string | null) {
  const task = await taskSsotService.getTask(taskId, subprojectId);
  if (!task) {
    return { task: null, workflowRun: null };
  }
...
function appendTaskArtifactLink(
  task: NonNullable<Awaited<ReturnType<typeof taskSsotService.getTask>>>,
  artifactPath: string,
) {
  if (task.artifactLinks.some((artifact) => artifact.artifactPath === artifactPath)) {
    return task;
  }
  return {
    ...task,
    artifactLinks: [
      ...task.artifactLinks,
      {
        ...
        artifactId: `${task.taskId}-artifact-${task.artifactLinks.length + 1}`,
        artifactPath,
        roleInTask: 'review-evidence' as const,
      },
    ],
...
async function backwriteWorkflowRunArtifact(input: {
  workflowRun: NonNullable<Awaited<ReturnType<typeof orchestratorRuntime.loadRun>>>;
  taskId: string;
  currentStage: string;
  artifactPath: string;
  detail: string;
  ...
  const nextStages = input.workflowRun.stages.map((stage) => stage.id === input.currentStage
    ? {
        ...stage,
        outputPaths: stage.outputPaths.includes(input.artifactPath)
          ? stage.outputPaths
          : [...stage.outputPaths, input.artifactPath],
      }
    : stage);
  const nextTasks = input.workflowRun.tasks.map((workflowTask) => workflowTask.id === input.taskId
    ? {
        ...workflowTask,
        artifactPaths: workflowTask.artifactPaths.includes(input.artifactPath)
          ? workflowTask.artifactPaths
          : [...workflowTask.artifactPaths, input.artifactPath],
      }
    : workflowTask);
  const updatedRun = {
    ...input.workflowRun,
  ...
  await memoryService.saveRunSnapshot(input.workflowRun.id, updatedRun);
  await memoryService.appendEvent(
    input.workflowRun.id,
    {
      id: `${input.workflowRun.id}-${input.taskId}-${path.basename(input.artifactPath).replace(/\W+/g, '-')}`,
...
async function executeTaskFrontendBrowserVerification(task: NonNullable<Awaited<ReturnType<typeof taskSsotService.getTask>>>, subprojectId: string | null) {
  ...
  if (task.sourceType === 'workflow-run-task' && task.sourceRef) {
    try {
      const loadedRun = await orchestratorRuntime.loadRun(task.sourceRef, subprojectId);
      workflowRun = await backwriteWorkflowRunArtifact({
        workflowRun: loadedRun,
        taskId: task.taskId,
  ...
  const nextTask = appendTaskArtifactLink(task, result.outputPath);
  return {
    browserRun: {
      taskId: task.taskId,
      ...result,
...
app.get('/api/skills', async (req, res) => {
  ...
  res.json({
    ...
    byMainline,
    designTooling: {
      packageName: 'ant-design-family-frontend',
      command: 'repo-skill',
      status:
        items.find((skill) => skill.id === 'ant-design-family-frontend')?.deployment.status ?? 'manual',
      statusPath: 'docs/operations/pmaios-ant-design-family-frontend-design.md',
      localGuidePath: 'skills/ant-design-family-frontend/SKILL.md',
    },
  });
});
...
app.get('/api/task-ssot/tasks/:taskId/final-state-validation', async (req, res) => {
  ...
  const { workflowRun } = await loadTaskWithWorkflowRun(req.params.taskId, subprojectId);
  res.json(finalStateValidationService.evaluateTask(task, workflowRun));
});

app.get('/api/task-ssot/tasks/:taskId/context-bundle', async (req, res) => {
  const subprojectId = normalizeSubprojectId(req.query.subprojectId);
  const task = await taskSsotService.getTask(req.params.taskId, subprojectId);
  if (!task) {
    res.status(404).json({ error: 'not_found' });
  ...
  res.json(contextInjectionService.buildBundle(task, workflowRun));
});

app.get('/api/task-ssot/tasks/:taskId/pipeline-launcher', async (req, res) => {
  const subprojectId = normalizeSubprojectId(req.query.subprojectId);
  const task = await taskSsotService.getTask(req.params.taskId, subprojectId);
  if (!task) {
    res.status(404).json({ error: 'not_found' });
  ...
  const plans = pipelineLauncherService.buildPlans(task, workflowRun);
  res.json({ items: plans, total: plans.length });
});

app.post('/api/task-ssot/tasks/:taskId/pipeline-launcher/:planId/trigger', async (req, res) => {
  const subprojectId = normalizeSubprojectId(req.query.subprojectId);
  const task = await taskSsotService.getTask(req.params.taskId, subprojectId);
  if (!task) {
    res.status(404).json({ error: 'not_found' });
  ...
  if (task.sourceType !== 'workflow-run-task' || !task.sourceRef) {
    res.status(400).json({ error: 'pipeline_launcher_requires_workflow_task' });
    return;
  }
...
app.post('/api/task-ssot/tasks/:taskId/frontend-browser-verification/run', async (req, res) => {
  try {
    ...
    const result = await executeTaskFrontendBrowserVerification(task, subprojectId);
    res.json(result.browserRun);
  } catch (error) {
    res.status(500).json({
...
app.post('/api/task-ssot/tasks/:taskId/auto-acceptance/run', async (req, res) => {
  try {
    const subprojectId = normalizeSubprojectId(req.query.subprojectId);
    const loaded = await loadTaskWithWorkflowRun(req.params.taskId, subprojectId);
    if (!loaded.task) {
      res.status(404).json({ error: 'not_found' });
      return;
    }
    ...
    let workflowRun = loaded.workflowRun;
    const browserVerification = finalStateValidationService.evaluateTask(task, workflowRun).browserVerification;
    let browserRun = null;

    if (browserVerification?.applicable) {
      const browserResult = await executeTaskFrontendBrowserVerification(task, subprojectId);
      browserRun = browserResult.browserRun;
      task = browserResult.task;
      workflowRun = browserResult.workflowRun ?? workflowRun;
    }

    const initialFinalState = finalStateValidationService.evaluateTask(task, workflowRun);
    const acceptanceArtifactPath = path.join('docs', 'review', `auto-acceptance-${task.taskId}.json`);
    const acceptanceArtifact = {
      taskId: task.taskId,
      automated: true,
      userVisible: initialFinalState.status === 'ready',
      generatedAt: new Date().toISOString(),
      browserRun,
      finalState: initialFinalState,
      summary:
        initialFinalState.status === 'ready'
          ? 'Automated acceptance passed. User review can now focus on business judgment instead of first-pass testing.'
          : 'Automated acceptance failed or requires rework. Do not hand this package to the user as a first-pass acceptance candidate.',
    };
    await store.writeJson(acceptanceArtifactPath, acceptanceArtifact);

    task = appendTaskArtifactLink(task, acceptanceArtifactPath);
    if (workflowRun) {
      workflowRun = await backwriteWorkflowRunArtifact({
        workflowRun,
        taskId: task.taskId,
        ...
        detail: `自动验收结果已写入 ${acceptanceArtifactPath}`,
        metadata: {
          acceptanceType: 'auto-acceptance',
          automated: true,
          userVisible: acceptanceArtifact.userVisible,
          finalStateStatus: initialFinalState.status,
        },
        status: initialFinalState.status === 'ready' ? 'ok' : 'warning',
        subprojectId,
      });
    }

    const finalState = finalStateValidationService.evaluateTask(task, workflowRun);
    res.json({
      ...acceptanceArtifact,
      userVisible: finalState.status === 'ready',
      finalState,
      summary:
        finalState.status === 'ready'
          ? 'Automated acceptance passed. User can review the delivered result instead of executing first-pass tests.'
          : 'Automated acceptance did not pass. The task remains blocked from user-facing acceptance.',
    });
  } catch (error) {
    res.status(500).json({
      error: 'auto_acceptance_failed',
      message: error instanceof Error ? error.message : String(error),
    });
  }
});
...
app.get('/api/scheduler/runtime', async (req, res) => {
  res.json(await schedulerRunService.buildRuntimeSummary(normalizeSubprojectId(req.query.subprojectId)));
});
...
app.get('/api/outbox/runtime', async (req, res) => {
...
app.post('/api/outbox/envelopes/:syncId/dispatch', async (req, res) => {
  const subprojectId = normalizeSubprojectId(req.body?.subprojectId ?? req.query.subprojectId);
  res.json(await outboxService.dispatchEnvelope(req.params.syncId, { subprojectId }));
});
...
app.post('/api/outbox/envelopes', async (req, res) => {
  ...
  res.status(201).json(
    await outboxService.enqueue({
      ...
      targetCategory: typeof req.body?.targetCategory === 'string' ? req.body.targetCategory : undefined,
      topicKey: typeof req.body?.topicKey === 'string' ? req.body.topicKey : undefined,
