...
export const DEMO_DEBUG_TASKS: DebugAutomationTask[] = [
  { id: 'debug-task-001', conversation_id: 'conv-001', media: '巨量引擎', debug_type: 'scan_qr', account: 'AD-12345', app_name: '游戏A', package_name: 'com.game.a', device: 'Pixel 6', environment: 'test', status: 'running_mobile_find_ad', current_stage: '手机端查找广告', current_step: 'step-3', requires_manual_confirm: false, created_at: iso(0), updated_at: iso(0) },
...
export const DEMO_DEBUG_STEPS: Record<string, DebugExecutionStep[]> = {
  'debug-task-001': [
    ...
    { id: 'step-3', task_id: 'debug-task-001', stage: 'running_mobile_find_ad', step_name: '查找广告', step_order: 3, status: 'running', log_summary: '正在查找...', started_at: iso(0) },
  ],
};
...
export const DEMO_MCP_SKILLS: McpSkill[] = [
  { id: 'skill-001', name: '归因数据', description: '查询广告归因数据，包括点击归因、曝光归因、归因窗口等', icon: '🎯', source: 'builtin', category: 'data', endpoint_url: 'https://attribution.internal/mcp', transport: 'streamable-http', auth_type: 'bearer_token', auth_config_template: { token: '' }, expected_tools: [{ name: 'query_attribution', description: '查询归因数据' }, { name: 'get_attribution_window', description: '获取归因窗口配置' }], installed: true, installed_server_id: 'mcp-001', tags: ['归因', '数据'], use_cases: ['排查归因差异', '查询归因窗口', '对比归因口径'], sort_order: 1, created_at: ts(30), updated_at: ts(0) },
  { id: 'skill-002', name: 'BI报表', description: '查询广告投放报表数据，消耗/展示/点击/转化等', icon: '📊', source: 'builtin', category: 'data', endpoint_url: 'https://report.internal/mcp', transport: 'streamable-http', auth_type: 'api_key', auth_config_template: { api_key: '' }, expected_tools: [{ name: 'query_report', description: '查询报表数据' }, { name: 'export_report', description: '导出报表' }], installed: true, installed_server_id: 'mcp-002', tags: ['报表', '数据'], use_cases: ['查询消耗报表', '对比投放数据', '导出报表'], sort_order: 2, created_at: ts(30), updated_at: ts(1) },
  { id: 'skill-003', name: '回传配置', description: '管理回传配置和状态，检查回传链路健康', icon: '🔄', source: 'builtin', category: 'integration', endpoint_url: 'https://postback.internal/mcp', transport: 'streamable-http', auth_type: 'bearer_token', auth_config_template: { token: '' }, expected_tools: [{ name: 'check_postback', description: '检查回传状态' }, { name: 'update_postback_config', description: '更新回传配置' }], installed: true, installed_server_id: 'mcp-003', tags: ['回传', '配置'], use_cases: ['排查回传异常', '配置回传地址', '检查回传率'], sort_order: 3, created_at: ts(20), updated_at: ts(5) },
  { id: 'skill-004', name: '自动联调', description: '执行广告联调自动化，支持扫码/深度链接/服务端联调', icon: '🔧', source: 'builtin', category: 'operation', endpoint_url: 'https://debug.internal/mcp', transport: 'streamable-http', auth_type: 'none', auth_config_template: {}, expected_tools: [{ name: 'start_debug', description: '启动联调' }, { name: 'get_debug_status', description: '获取联调状态' }], installed: true, installed_server_id: 'mcp-004', tags: ['联调', '自动'], use_cases: ['扫码联调', '深度链接测试', '服务端联调验证'], sort_order: 4, created_at: ts(15), updated_at: ts(0) },
  { id: 'skill-005', name: '知识库', description: '搜索广告知识库，指标定义/配置指南/排查方法', icon: '📚', source: 'builtin', category: 'data', endpoint_url: 'https://knowledge.internal/mcp', transport: 'streamable-http', auth_type: 'none', auth_config_template: {}, expected_tools: [{ name: 'search_knowledge', description: '搜索知识库' }], installed: true, installed_server_id: 'mcp-005', tags: ['知识库'], use_cases: ['查询指标定义', '搜索配置指南', '查找排查方法'], sort_order: 5, created_at: ts(30), updated_at: ts(0) },
  { id: 'skill-006', name: '监控配置', description: '管理广告监控配置，设置告警规则', icon: '👁', source: 'builtin', category: 'monitor', endpoint_url: 'https://monitor-config.internal/mcp', transport: 'streamable-http', auth_type: 'api_key', auth_config_template: { api_key: '' }, expected_tools: [{ name: 'get_monitor_config', description: '获取监控配置' }, { name: 'set_alert_rule', description: '设置告警规则' }], installed: false, tags: ['监控', '配置'], use_cases: ['设置消耗告警', '配置监控规则', '查看监控状态'], sort_order: 6, created_at: ts(15), updated_at: ts(10) },
  { id: 'skill-007', name: '游戏接入', description: '查询游戏接入报告和进度', icon: '🎮', source: 'builtin', category: 'data', endpoint_url: 'https://game-report.internal/mcp', transport: 'streamable-http', auth_type: 'bearer_token', auth_config_template: { token: '' }, expected_tools: [{ name: 'get_access_report', description: '获取接入报告' }], installed: false, tags: ['游戏', '报告'], use_cases: ['查询接入进度', '获取接入报告'], sort_order: 7, created_at: ts(10), updated_at: ts(3) },
  { id: 'skill-008', name: '素材分析', description: '分析广告素材效果，创意脚本解析，相似度匹配', icon: '🎨', source: 'builtin', category: 'analysis', endpoint_url: 'https://material.internal/mcp', transport: 'streamable-http', auth_type: 'api_key', auth_config_template: { api_key: '' }, expected_tools: [{ name: 'analyze_material', description: '分析素材效果' }, { name: 'find_similar', description: '查找相似素材' }], installed: false, tags: ['素材', '分析'], use_cases: ['分析素材效果', '查找相似素材', '生成素材报告'], sort_order: 8, created_at: ts(10), updated_at: ts(2) },
];
