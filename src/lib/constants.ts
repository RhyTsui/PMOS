...
export const MOCK_WORKSPACE: WorkspaceResponse = {
  ...
  user_name: '广告支持-小乔',
  status_summary: '可用',
  quick_modes: ['使用帮助', '需求沟通', '问题排查', '广告联调', '自动报表'],
  recent_tasks: [],
...
export const MOCK_INITIAL_MESSAGES: Message[] = [
  {
    ...
    role: 'system',
    content: '欢迎使用智投chat工作台。您可以直接描述问题或需求，我会自动识别并进入对应业务流。',
...
export function generateAssistantReply(routing: RoutingDecision, userMessage: string): string {
  if (!routing.is_business_related) {
    return '您好！我是智投chat工作台。您可以直接描述广告相关的问题、需求或异常，我会自动帮您进入对应流程。';
  }
...
export const MOCK_MCP_SERVERS: McpServerConfig[] = [
  {
    id: 'mcp-attribution',
    name: '归因数据服务',
    description: '提供广告归因数据查询、归因窗口配置、归因报告生成能力。支持巨量/抖音/腾讯/快手多平台归因数据获取。',
    category: 'data',
    endpoint_url: 'https://attribution.internal.example.com/mcp',
    transport: 'streamable-http',
    auth_type: 'bearer_token',
    auth_config: { token: '••••••••••••' },
    status: 'connected',
    enabled: true,
    business_domains: ['归因', '数据分析'],
    bound_agents: ['diagnosis', 'help'],
    tags: ['归因', '数据', '只读'],
    health_check_url: 'https://attribution.internal.example.com/health',
    last_health_check_at: Date.now() - 300000,
    tools: [
      {
        tool_id: 'query_attribution_data',
        name: 'query_attribution_data',
        description: '查询归因数据，支持按日期/媒体/应用/事件类型筛选',
        input_schema: { date_range: { type: 'string' }, media: { type: 'string' }, app_id: { type: 'string' } },
        enabled: true,
        bound_agents: ['help', 'diagnosis'],
        access_mode: 'read',
        call_count: 1247,
        last_called_at: Date.now() - 3600000,
      },
      {
        tool_id: 'get_attribution_window',
        name: 'get_attribution_window',
        description: '获取归因窗口配置（点击归因/浏览归因窗口期）',
        input_schema: { media: { type: 'string' }, app_id: { type: 'string' } },
        enabled: true,
        bound_agents: ['help', 'diagnosis'],
        access_mode: 'read',
        call_count: 589,
        last_called_at: Date.now() - 7200000,
      },
      {
        tool_id: 'generate_attribution_report',
        name: 'generate_attribution_report',
        description: '生成归因分析报告，含趋势对比和异常标注',
        input_schema: { date_range: { type: 'string' }, metrics: { type: 'array' } },
        enabled: true,
        bound_agents: ['diagnosis'],
        access_mode: 'read',
        call_count: 234,
        last_called_at: Date.now() - 86400000,
      },
    ],
  ...
  },
  {
    id: 'mcp-report',
    name: 'BI报表服务',
    description: '提供BI报表生成、指标趋势查询、多维对比分析能力。对接内部数据仓库和广告平台API。',
    category: 'data',
    endpoint_url: 'https://bi-report.internal.example.com/mcp',
    transport: 'streamable-http',
    auth_type: 'api_key',
    auth_config: { api_key: '••••••••••••' },
    status: 'connected',
    enabled: true,
    business_domains: ['报表', '数据分析'],
    bound_agents: ['help', 'analysis'],
    tags: ['报表', 'BI', '只读'],
    health_check_url: 'https://bi-report.internal.example.com/health',
    last_health_check_at: Date.now() - 600000,
    tools: [
      {
        tool_id: 'query_metric_trend',
        name: 'query_metric_trend',
        description: '查询指标趋势数据，支持ROI/LTV/CPA/CVR等核心指标',
        input_schema: { metric: { type: 'string' }, date_range: { type: 'string' }, granularity: { type: 'string' } },
        enabled: true,
        bound_agents: ['help', 'diagnosis'],
        access_mode: 'read',
        call_count: 2156,
        last_called_at: Date.now() - 1800000,
      },
      {
        tool_id: 'generate_bi_report',
        name: 'generate_bi_report',
        description: '生成BI报表，支持自定义维度和指标组合',
        input_schema: { dimensions: { type: 'array' }, metrics: { type: 'array' }, date_range: { type: 'string' } },
        enabled: true,
        bound_agents: ['help'],
        access_mode: 'read',
        call_count: 876,
        last_called_at: Date.now() - 43200000,
      },
      {
        tool_id: 'compare_multi_dimension',
        name: 'compare_multi_dimension',
        description: '多维对比分析，支持跨媒体/跨应用/跨时间段对比',
        input_schema: { dimension: { type: 'string' }, entities: { type: 'array' }, metrics: { type: 'array' } },
        enabled: false,
        bound_agents: ['diagnosis'],
        access_mode: 'read',
        call_count: 123,
        last_called_at: Date.now() - 172800000,
      },
    ],
    created_at: Date.now() - 2592000000,
  ...
  },
  {
    id: 'mcp-postback',
    name: '回传监控服务',
    description: '提供广告回传状态监控、事件映射查询、回传日志检索能力。支持回传配置的增删改查。',
    category: 'data',
    endpoint_url: 'https://postback.internal.example.com/mcp',
    transport: 'sse',
    auth_type: 'api_key',
    auth_config: { api_key: '••••••••••••' },
    status: 'connected',
    enabled: true,
    business_domains: ['回传', '联调'],
    bound_agents: ['demand', 'diagnosis'],
    tags: ['回传', '监控', '读写'],
    tools: [
      {
        tool_id: 'query_postback_status',
        name: 'query_postback_status',
        description: '查询回传状态，包含成功/失败/延迟统计',
        input_schema: { app_id: { type: 'string' }, media: { type: 'string' }, date_range: { type: 'string' } },
        enabled: true,
        bound_agents: ['diagnosis', 'demand'],
        access_mode: 'read',
        call_count: 3421,
        last_called_at: Date.now() - 900000,
      },
      {
        tool_id: 'query_event_mapping',
        name: 'query_event_mapping',
        description: '查询事件映射配置（业务事件↔媒体事件）',
        input_schema: { app_id: { type: 'string' }, media: { type: 'string' } },
        enabled: true,
        bound_agents: ['help', 'demand'],
        access_mode: 'read',
        call_count: 1567,
        last_called_at: Date.now() - 5400000,
      },
      {
        tool_id: 'update_event_mapping',
        name: 'update_event_mapping',
        description: '更新事件映射配置',
        input_schema: { app_id: { type: 'string' }, media: { type: 'string' }, mappings: { type: 'array' } },
        enabled: true,
        bound_agents: ['demand'],
        access_mode: 'write',
        call_count: 234,
      ...
      {
        tool_id: 'search_postback_logs',
        name: 'search_postback_logs',
        description: '搜索回传日志，按时间/状态/错误类型筛选',
        input_schema: { app_id: { type: 'string' }, status: { type: 'string' }, date_range: { type: 'string' } },
        enabled: true,
        bound_agents: ['diagnosis'],
        access_mode: 'read',
        call_count: 2890,
        last_called_at: Date.now() - 2700000,
      },
    ],
    created_at: Date.now() - 2160000000,
    updated_at: Date.now() - 43200000,
  },
  {
    id: 'mcp-monitor',
    name: '监控告警服务',
    description: '提供广告操作监控、上报监控、回推监控和异常告警能力。支持阈值配置和告警推送。',
    category: 'data',
    endpoint_url: 'https://monitor.internal.example.com/mcp',
    transport: 'streamable-http',
    auth_type: 'bearer_token',
    auth_config: { token: '••••••••••••' },
    status: 'disconnected',
    enabled: false,
    business_domains: ['监控', '告警'],
    bound_agents: ['diagnosis'],
    tags: ['监控', '告警', '只读'],
    error_message: '连接超时: 服务器未响应',
    tools: [
      {
        tool_id: 'query_operation_metrics',
        name: 'query_operation_metrics',
        description: '查询广告操作指标（消耗/展示/点击/转化）',
        input_schema: { media: { type: 'string' }, date_range: { type: 'string' } },
        enabled: true,
        bound_agents: ['help', 'diagnosis'],
        access_mode: 'read',
        call_count: 0,
      ...
      {
        tool_id: 'query_alert_rules',
        name: 'query_alert_rules',
        description: '查询告警规则配置',
    ...
    created_at: Date.now() - 1728000000,
    updated_at: Date.now() - 259200000,
  },
  {
    id: 'mcp-debug',
    name: '联调执行服务',
    description: '提供广告联调自动化执行能力：设备控制、广告识别、截图获取、联调状态机驱动。',
    category: 'function',
    endpoint_url: 'https://debug-automation.internal.example.com/mcp',
    transport: 'streamable-http',
    auth_type: 'oauth2',
    auth_config: { client_id: 'debug-mcp-client', client_secret: '••••••••••••' },
    status: 'connected',
    enabled: true,
    business_domains: ['联调'],
    ...
    tools: [
      {
        tool_id: 'create_debug_task',
        name: 'create_debug_task',
        description: '创建自动联调任务',
        input_schema: { media: { type: 'string' }, app_id: { type: 'string' }, device_id: { type: 'string' } },
        enabled: true,
        bound_agents: ['debugging'],
        access_mode: 'write',
        call_count: 67,
      ...
      {
        tool_id: 'execute_debug_step',
        name: 'execute_debug_step',
        description: '执行联调步骤（扫码/找广告/点击/拉起/轮询）',
        input_schema: { task_id: { type: 'string' }, step: { type: 'string' } },
        enabled: true,
        bound_agents: ['debugging'],
        access_mode: 'write',
        call_count: 312,
      ...
      {
        tool_id: 'get_debug_screenshot',
        name: 'get_debug_screenshot',
        description: '获取联调截图（Web页面/设备屏幕）',
        input_schema: { task_id: { type: 'string' }, phase: { type: 'string' } },
        enabled: true,
        bound_agents: ['debugging'],
        access_mode: 'read',
        call_count: 189,
      ...
      {
        tool_id: 'takeover_debug_task',
        name: 'takeover_debug_task',
        description: '人工接管联调任务',
        input_schema: { task_id: { type: 'string' }, reason: { type: 'string' } },
        enabled: true,
        bound_agents: ['debugging'],
        access_mode: 'write',
        call_count: 12,
        last_called_at: Date.now() - 604800000,
      },
    ],
  ...
  },
  {
    id: 'mcp-material',
    name: '素材分析服务',
    description: '提供广告素材解析、脚本拆解、相似度匹配和素材报告生成能力。',
    category: 'function',
    endpoint_url: 'https://material.internal.example.com/mcp',
    transport: 'sse',
    auth_type: 'api_key',
    auth_config: { api_key: '••••••••••••' },
    status: 'error',
    enabled: false,
    business_domains: ['素材'],
    bound_agents: ['analysis'],
    tags: ['素材', '分析', '只读'],
    error_message: '认证失败: API Key 已过期',
    tools: [
      {
        tool_id: 'parse_creative_script',
        name: 'parse_creative_script',
        description: '解析创意脚本，提取场景/角色/对话/时长',
        input_schema: { material_url: { type: 'string' } },
        enabled: true,
        bound_agents: ['help'],
        access_mode: 'read',
        call_count: 45,
      ...
      {
        tool_id: 'match_similar_material',
        name: 'match_similar_material',
        description: '匹配相似素材，返回相似度排名',
        input_schema: { material_url: { type: 'string' }, top_k: { type: 'number' } },
        enabled: true,
        bound_agents: ['help'],
        access_mode: 'read',
        call_count: 23,
