...
const BUILTIN_MCP_SERVERS: McpServerConfig[] = [
  ...
  },
  {
    ...
    name: '巨量引擎 MCP',
    description: '承载巨量侧应用和应用分包只读查询能力；通过包名+终端匹配智投应用与巨量应用，v1 不创建分包。',
    category: 'data',
    endpoint_url: '',
    transport: 'sse',
    auth_type: 'bearer_token',
    ...
    tags: ['P0', '巨量引擎', 'SSE', '分包', '只读', '真实MCP'],
    tools: [
      {
        tool_id: 'tools_app_management_android_app_list_v2',
        name: 'tools_app_management_android_app_list_v2',
        ...
        input_schema: {
          ...
          properties: {
            ...
            default_account: { type: 'string', description: '后台配置的巨量默认账户' },
          },
        },
        enabled: true,
        bound_agents: ['debugging'],
      ...
      {
        tool_id: 'oceanengine.app_package_query',
        name: 'oceanengine.app_package_query',
        description: '查询巨量应用下已有应用分包、审核状态和可联调状态；只返回已有结果，不创建分包。',
        input_schema: {
          type: 'object',
          required: ['oceanengine_app_id', 'terminal'],
          properties: {
            oceanengine_app_id: { type: 'string', description: '巨量应用ID' },
            package_name: { type: 'string', description: '智投应用包名，用于二次匹配' },
            terminal: { type: 'string', description: 'android 或 ios' },
            default_account: { type: 'string', description: '后台配置的巨量默认账户' },
          },
        },
