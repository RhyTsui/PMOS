...
export function AutoDebugWorkbench({ conversationId, onOpenContext }: AutoDebugWorkbenchProps) {
  ...
  const validations = useMemo(() => {
    ...
    return [
      ...
      { label: '账户应用校验', ok: valuePresent(channel.game_package || game.package_name), detail: '会确认默认账户下是否可使用目标应用，必要时提示处理' },
      { label: '数据上报校验', ok: true, detail: '激活/注册查真实上报，付费按模拟付费联调' },
      { label: '联调能力', ok: true, detail: '联调执行和数据检查能力可用' },
    ];
  }, [form, matchedConfig]);
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div>
            ...
            <div style={{ marginTop: 4, fontSize: 12, color: c.textMuted, lineHeight: 1.6 }}>
              只需要确认项目、媒体和终端；应用共享、事件资产、设备、回传查看位置和动作关键词会自动准备。
            </div>
          </div>
