...
export class TaskSsotService {
  ...
  private syncTask(previous: TaskSsotTask | undefined, task: TaskNode, mcpState: McpContextState): TaskSsotTask {
    ...
    return {
      ...
      title: task.label,
      summary,
      collaborationLevel: previous?.collaborationLevel ?? 'L0',
      status,
  ...
  private syncWorkflowTask(previous: TaskSsotTask | undefined, run: WorkflowRun, task: WorkflowTask, events: WorkflowEvent[]): TaskSsotTask {
    ...
    return {
      ...
      title: task.title,
      summary,
      collaborationLevel: previous?.collaborationLevel ?? 'L1',
      status,
      currentStage: task.stageId,
      currentOwnerAgentId,
  ...
  private resolveGateChecks(input: {
    ...
    const checks: TaskSsotTask['gateChecks'] = [
      ...
      },
      {
        ...
        reason:
          input.artifactLinks.length > 0
            ? 'Repository-backed artifacts are linked to the task.'
            : input.status === 'completed'
              ? 'Task is completed but no repository-backed artifact link was written back.'
              : 'Task has not written back repository-backed artifacts yet.',
        evidencePaths: workingOutputPaths,
        checkedAt,
      },
    ];
