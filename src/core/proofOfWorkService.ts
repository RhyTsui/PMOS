...
export class ProofOfWorkService {
  constructor(
    ...
    private readonly contextInjectionService = new ContextInjectionService(),
    private readonly pipelineLauncherService = new PipelineLauncherService(),
  ) {}
  ...
  buildBundle(input: {
    ...
    const contextBundle = this.contextInjectionService.buildBundle(input.task, input.workflowRun);
    const pipelineLauncher = this.pipelineLauncherService.buildPlans(input.task, input.workflowRun);
