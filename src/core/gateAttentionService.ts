...
export class GateAttentionService {
  ...
  private buildSuggestedActions(input: {
    ...
    if (input.gateId === 'specialist-activation-gate') {
      actions.push(
        input.status === 'block'
          ? 'Backfill the missing specialist reviewer roles before continuing delivery.'
          : 'Replace implicit specialist activation defaults with runtime-backed reviewer evidence.',
      );
    }
  ...
  private buildOperatorEntries(input: {
    ...
    if (input.gateId === 'specialist-activation-gate') {
      entries.push(this.createEntry(input, 'resume-current-stage', 'Refresh Specialist Evidence'));
    }
