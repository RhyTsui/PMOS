...
const STAGE_REQUIRED_REVIEW_ROLES: Record<string, string[]> = {
  'requirements-document': ['Solution-Optimality Review', 'Technical Review', 'Research Review'],
  'functional-specification': ['Development Review', 'Technical Review', 'Research Review'],
  'design-document': ['Design Review', 'Product Review', 'Research Review'],
  'frontend-page': ['Design Review', 'Frontend Review', 'Research Review'],
  'data-schema': ['Data Review', 'Technical Review'],
  'backend-api': ['Backend Review', 'Development Review'],
  'frontend-backend-integration': ['Delivery Review', 'Development Review', 'Research Review'],
};

export class SpecialistActivationService {
  resolveAssignedRolesForStage(stageId?: string | null): string[] {
    return [...(STAGE_REQUIRED_REVIEW_ROLES[stageId ?? 'frontend-backend-integration'] ?? [])];
  }
  ...
  resolveActivatedRoles(input: {
    ...
    taskAssignmentRoles?: string[];
    workflowSignals?: WorkflowReviewSignal[];
  }): string[] {
    const stageId = input.stageId ?? 'frontend-backend-integration';
    const activated = new Set<string>();
    const directReviewRoles = new Set(input.taskAssignmentRoles ?? []);
    const signalRoles = new Set(
      (input.workflowSignals ?? []).flatMap((signal) => [...(signal.activatedSpecialistRoles ?? [])]),
    );
