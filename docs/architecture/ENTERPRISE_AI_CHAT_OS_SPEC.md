...
## 23. 工程实施协作规范补充：反局部收敛与未上线能力闭环

> 依据：`docs/sources/inbox/小乔智投工程实施协作规范补充.docx`  
> 适用阶段：未上线、待能力闭环  
> 适用范围：小乔智投（`subprojects/ad`）所有核心链路任务

### 23.1 阶段定位与总原则

当前目标不是稳定维护已上线系统，而是尽快完成未上线能力的系统闭环，使核心能力达到浏览器可验收、可灰度、可上线状态。

在此阶段：

- **已发现问题必须进入同一批闭环清单**，不允许只修局部导致其他已知问题遗漏。
- **不允许模型自行判断"先不做""后续再说"**，除非明确说明风险并等待用户确认。
- **不能只追求最小 diff，而要追求链路真实跑通**。
- **每轮任务必须从用户已列问题出发建立完整 checklist**，每个项标明：已修 / 未修 / 不修原因 / 验收方式。
- **所有修复必须最终回到真实 E2E 和 trace dump 验证**。

### 23.2 对 AI Agent 的特别约束

AI Agent（Claude Code / Codex CLI 等）在执行时必须遵守：

| 禁止行为 | 允许的替代做法 |
|----------|----------------|
| "本轮先不处理"但没有用户确认 | 标注该项需要单独方案，写入 backlog，说明原因和验收口径 |
| "非阻断，后续处理"但没有记录 | 标注 feature flag 依赖，登记到 deferred list |
| "不是当前主因"后直接丢弃 | 保留在问题清单中，标明优先级和阻塞关系 |
| 只处理模型认为最关键的一项 | 用户列出的每一项都进入任务清单并给出处理状态 |
| 只修当前报错，不处理同批已知链路问题 | 系统性治理覆盖设计文档、运行链路、数据契约、代码实现、trace、UI、单测、E2E |

### 23.3 架构红线

在未上线阶段，以下行为视为架构违规：

1. **隐藏 warning** 而不修真实数据流
2. **删除 UI 展示** 但不修链路
3. **跳过 preflight / resolver** 伪装修复
4. **强行 fallback** 成通用回答
5. **把 missing_input 改成 success** 而不处理缺失输入
6. **把 skipped 从 UI 删除** 但实际仍错误执行
7. **修复只覆盖单一路径**，真实链路仍不生效
8. **新旧并行架构继续共存**，代码改动看似收敛实际能力仍无法上线

### 23.4 LLM 调用点治理

所有 LLM 调用点（route understanding、filter correction、structured understanding、four-layer thinking chain、QueryContract review、capability discovery assist、tool selection review、resolver disambiguation review、operation risk review、mcp-agent review/replan、answer composition）必须：

- 记录输入 schema 和输出 schema
- 记录 Prompt 版本和模型选择
- 记录调用频率和失败策略
- 在 trace 中有可观测证据
...
当前用户侧只展示：主回答、必要图表/表格、文本追问、trace 链接。

禁止展示：任务候选检查、多工具编排未覆盖、knowledge skipped、内部 role review、调用过程组件、领域信号 evidence_only、report_query/data_query 内部不一致提示。

### 23.6 验收策略

- 单元测试 + 关键链路 dump + 浏览器真实 E2E + traceId
- 对比修复前后的用户可见结果
- 提供未修问题清单和下一步阻塞清单
- **E2E 不通过，不允许宣称完成**
...
当前阶段不是"少改一点避免风险"，而是"已知问题必须闭环，否则能力无法上线"。任何局部修复都必须证明其进入真实主链，否则视为未完成。
