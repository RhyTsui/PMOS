...
│   ├── page.tsx           # Main workspace (three-column layout + support layer)
│   ├── admin/page.tsx     # Prompt management page (3-column layout)
│   └── globals.css        # Global styles + XiaoQiao theme
├── components/
│   ├── cognitive/         # Conversation Hub components
│   │   ├── AgentDock.tsx         # Agent switcher (4 core flows + 3 special pages)
│   │   ├── ChatContainer.tsx     # Main chat area with routing
│   │   ├── Header.tsx            # Workspace status bar
│   │   ├── InputArea.tsx         # Message input with attachment & context display
│   │   ├── MessageBubble.tsx     # Message with routing/task badges
...
│       ├── MetricCard.tsx
│       └── StatusBadge.tsx
│   ├── api/                # Backend API routes (Agent + Tools)
│   │   └── chat/
│   │       └── route.ts    # POST /api/chat - LLM streaming + knowledge search + web search
│   ├── hooks/
│   ├── useAgent.tsx       # Agent context (workspace, tasks, results, attachments, evidence, missing fields)
│   └── useConversation.ts # Conversation management with routing & task creation
├── lib/
│   └── constants.ts       # Business objects, mock data, agent configs, enhanced schemas
└── types/
...
## Workspace Layout (三栏工作台 + 会话支撑层)

- **左栏**: 主对话区 (消息流 + 路由标签 + 附件条 + 上下文摘要)
- **中栏**: 结构化结果区 (帮助结果/需求单/排查报告/联调报告 + 证据卡 + 附件摘要)
- **右栏**: 历史任务区 (任务列表 + 状态 + 最近结果摘要)
...
- Mock data matches real business objects from project documentation
- Mock objects are structured to match the backend API contract
- Current stage uses mock contract; real API integration planned for Phase 2
- `.env.pre.local` and database config entries are reserved for future environment switching
