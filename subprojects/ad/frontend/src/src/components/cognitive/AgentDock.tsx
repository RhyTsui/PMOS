...
function AgentDockItem({ agentId, isActive, onClick }: AgentDockItemProps) {
  ...
  return (
    ...
    >
      ...
      {isActive && <div className="w-1 h-4 rounded-full bg-[#175CD3]" />}
    </button>
  );
}

const DOCK_AGENTS: AgentType[] = ['report'];
    ...
    <aside className="w-56 flex-shrink-0 border-r border-white/[0.06] bg-[#0a0e1a] flex flex-col">
      <div className="flex-1 overflow-y-auto py-3 px-2 space-y-1">
        <div className="px-3 py-1.5">
          <span className="text-[11px] font-medium text-white/20 uppercase tracking-wider">问数服务</span>
        </div>
        {DOCK_AGENTS.map((agentId) => (
          <AgentDockItem
            key={agentId}
            agentId={agentId}
