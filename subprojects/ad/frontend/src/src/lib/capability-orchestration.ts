...
const GOVERNED_BUILTIN_CAPABILITIES: CapabilityManifest[] = [
  {
    capabilityId: 'builtin.help_qa',
    displayName: '使用帮助与知识解释',
    description: '回答能力范围、配置准备、字段口径和使用方法类问题；只提供候选能力，不直接授权执行。',
    provider: 'builtin',
    capabilityType: 'general',
    capabilityPurpose: 'dictionary_lookup',
    center: 'conversation',
    serviceLine: 'help',
    automationLevel: 'L0',
    owner: 'ai-chat-governance',
    governanceVersion: 'capability-seed/2026-06-23',
    evidenceNeed: ['knowledge_base', 'source_ref_or_model_only_boundary'],
    outputSurface: ['chat_answer'],
    routeRuleRefs: ['capability-route-rule:help_qa'],
    fallbackPolicy: 'clarify',
    executionClass: 'read_only',
    supportedSemanticTasks: ['explain_field_or_value', 'general_chat'],
    supportedServiceIntents: ['help_qa', 'field_definition', 'knowledge_answer', 'general_chat'],
    toolPurpose: 'help_lookup',
    primaryGoal: '回答用户问题并给出可继续处理的下一步。',
    requiredInputs: [],
    optionalInputs: ['question', 'context_refs'],
    inputContract: { optionalFields: ['question', 'context_refs'], description: '用户问题与可用上下文引用。' },
    outputContract: { contractType: 'semantic_result', requiredFields: ['answer_markdown', 'next_actions'] },
    riskLevel: 'low',
    dataDomain: 'chat',
    supports: { metrics: [], dimensions: [], identifierTypes: [], granularity: [], views: ['summary', 'detail'] },
    source: { sourceType: 'builtin', toolName: 'answer_composer.help_qa' },
  },
  {
    capabilityId: 'builtin.requirement_drafting',
    displayName: '需求梳理',
    description: '将用户目标整理为需求草案、缺口和下一步；只提供候选能力，不直接授权执行。',
    provider: 'builtin',
    capabilityType: 'general',
    capabilityPurpose: 'workflow_execution',
    center: 'conversation',
    serviceLine: 'requirement',
    automationLevel: 'L1',
    owner: 'ai-chat-governance',
    governanceVersion: 'capability-seed/2026-06-23',
    evidenceNeed: ['user_input', 'context_refs'],
    outputSurface: ['chat_answer', 'task_center'],
    routeRuleRefs: ['capability-route-rule:requirement_drafting'],
    fallbackPolicy: 'clarify',
    executionClass: 'workflow',
    supportedSemanticTasks: ['draft_requirement'],
    supportedServiceIntents: ['light_requirement'],
    toolPurpose: 'draft_generation',
    primaryGoal: '生成可确认的需求草案与缺失信息。',
    requiredInputs: ['goal'],
    optionalInputs: ['constraints', 'deadline', 'stakeholders'],
    inputContract: { requiredFields: ['goal'], optionalFields: ['constraints', 'deadline', 'stakeholders'], description: '需求目标、约束与交付偏好。' },
    outputContract: { contractType: 'semantic_result', requiredFields: ['business_summary', 'missing_fields', 'next_actions'] },
    riskLevel: 'medium',
    dataDomain: 'chat',
    supports: { metrics: [], dimensions: [], identifierTypes: [], granularity: [], views: ['summary', 'detail'] },
    source: { sourceType: 'builtin', toolName: 'answer_composer.requirement_drafting' },
  },
  {
    capabilityId: 'builtin.issue_diagnosis',
    displayName: '问题排查证据规划',
    description: '规划异常排查所需证据、限制与下一步；只提供候选能力，不直接授权执行。',
    provider: 'builtin',
    capabilityType: 'workflow',
    capabilityPurpose: 'diagnostic_evidence',
    center: 'delivery_integration',
    serviceLine: 'diagnosis',
    automationLevel: 'L1',
    owner: 'ai-chat-governance',
    governanceVersion: 'capability-seed/2026-06-23',
    evidenceNeed: ['tool_result', 'logs', 'user_context'],
    outputSurface: ['chat_answer', 'task_center'],
    routeRuleRefs: ['capability-route-rule:issue_diagnosis'],
    fallbackPolicy: 'clarify',
    executionClass: 'diagnostic',
    supportedSemanticTasks: ['diagnose_data_issue'],
    supportedServiceIntents: ['issue_diagnosis'],
    toolPurpose: 'evidence_fetch',
    primaryGoal: '形成可审计的问题排查计划。',
    requiredInputs: ['problem', 'scope'],
    optionalInputs: ['time_range', 'source_refs', 'sample'],
    inputContract: { requiredFields: ['problem', 'scope'], optionalFields: ['time_range', 'source_refs', 'sample'], description: '问题现象、范围与证据材料。' },
    outputContract: { contractType: 'semantic_result', requiredFields: ['business_summary', 'evidence_refs', 'next_actions'] },
    riskLevel: 'medium',
    dataDomain: 'workflow',
    supports: { metrics: [], dimensions: [], identifierTypes: [], granularity: [], views: ['diagnosis', 'summary'] },
    source: { sourceType: 'builtin', toolName: 'evidence_planner.issue_diagnosis' },
  },
  {
    capabilityId: 'builtin.integration_workflow',
    displayName: '联调流程规划',
    description: '规划联调检查、配置确认和失败归一化；只提供候选能力，不直接授权执行。',
    provider: 'builtin',
    capabilityType: 'workflow',
    capabilityPurpose: 'workflow_execution',
    center: 'delivery_integration',
    ...
    owner: 'ai-chat-governance',
    governanceVersion: 'capability-seed/2026-06-23',
    evidenceNeed: ['tool_result', 'task_state', 'logs'],
    outputSurface: ['chat_answer', 'task_center'],
    routeRuleRefs: ['capability-route-rule:integration_workflow'],
    fallbackPolicy: 'clarify',
    executionClass: 'workflow',
    supportedSemanticTasks: ['execute_workflow'],
    supportedServiceIntents: ['integration_workflow', 'system_operation'],
    toolPurpose: 'integration_run',
    primaryGoal: '形成联调处理计划并确认高风险动作。',
    requiredInputs: ['scope'],
    optionalInputs: ['environment', 'sample', 'approval_ref'],
    inputContract: { requiredFields: ['scope'], optionalFields: ['environment', 'sample', 'approval_ref'], description: '联调范围、环境和审批引用。' },
    outputContract: { contractType: 'semantic_result', requiredFields: ['business_summary', 'runtime_refs', 'next_actions'] },
    riskLevel: 'high',
    dataDomain: 'workflow',
    supports: { metrics: [], dimensions: [], identifierTypes: [], granularity: [], views: ['summary', 'detail'] },
    source: { sourceType: 'builtin', toolName: 'workflow_planner.integration' },
  },
];
export function buildCapabilityManifest(servers: McpServerConfig[]): CapabilityManifest[] {
  ...
  return [...GOVERNED_BUILTIN_CAPABILITIES, ...capabilities];
}
