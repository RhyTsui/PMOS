...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!task || task.created_by !== scope.key) return NextResponse.json({ error: 'Task not found' }, { status: 404 });
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const task = await updateScheduledTask(id, { ...body, created_by: current.created_by });
