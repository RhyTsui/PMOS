...
export async function POST(request: NextRequest) {
  try {
    ...
    const workflowTask = await createWorkflowTask({
      conversation_id: `report-draft-${Date.now()}`,
      task_type: 'report_draft_generate',
      workflow_level: 'light',
      owner_type: 'xiaoqiao',
      title: template.name,
      summary: `${template.name} / ${reportDate}`,
      route_reason: 'admin-report-draft-generate',
    ...
    const workflowResult: WorkflowResult = {
      result_id: `report-draft-${Date.now()}`,
