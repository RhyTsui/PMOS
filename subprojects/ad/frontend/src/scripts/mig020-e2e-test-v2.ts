/**
 * MIG-020 E2E验收测试 - 使用已登录的浏览器会话
 */
import { chromium, type BrowserContext, type Page } from 'playwright';
import { writeFileSync } from 'node:fs';

const BASE_URL = 'http://10.236.14.27:8002';
const USER_DATA_DIR = 'E:/AI/ai-os/.runtime/chrome-e2e-mig020';
...
async function main(): Promise<void> {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`MIG-020 E2E验收测试 (使用已登录会话)`);
  console.log(`${'='.repeat(60)}\n`);

  console.log('[INFO] 连接到已登录的浏览器...');
  ...
  try {
    ...
    if (new URL(page.url()).pathname.startsWith('/login')) {
      console.log('\n[WARN] 未检测到登录状态，请等待10秒后重试...');
      await new Promise(resolve => setTimeout(resolve, 10000));
      await page.goto(BASE_URL, { waitUntil: 'domcontentloaded', timeout: 30000 });
      if (new URL(page.url()).pathname.startsWith('/login')) {
        throw new Error('未检测到登录状态，请先在浏览器中登录');
      }
    }
