...
const TESTS = [
  { id: 'MIG-006', input: '能不能选一个事件作为付费回传', kw: ['事件', '付费', '回传'] },
  { id: 'MIG-007', input: '有效是哪个点位', kw: ['有效', '点位', '指标'] },
  { id: 'MIG-008', input: '素材报表的未知是什么', kw: ['素材', '报表', '未知'] },
];

(async () => {
  log('=== BATCH TEST: MIG-006/007/008 (UI only) ===');
  ...
  fs.writeFileSync(path.join(REPORT_DIR, 'batch-006-008-ui-report.json'), JSON.stringify({ timestamp: new Date().toISOString(), tests: results }, null, 2));
