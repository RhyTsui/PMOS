...
- explicit component-baseline choice when the product is chat-like, including whether `Ant Design X` is the default conversation-region library
...
- If the product is conversation-first, did the package explicitly decide whether chat-region components default to `Ant Design X` and whether non-chat regions still follow enterprise-shell rules?
